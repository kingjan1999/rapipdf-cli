  ...
  if (spec.info) {
    ...
    if (spec.info.contact) {
      ...
      if (spec.info.termsOfService) {
        termsOfService = {
          text: [
            { text: `\n${localize.termsOfService}: `, style: ["b", "small"] },
            {
      ...
      contactDef = [
        ...
        {
          text: [contactName, contactEmail, contactUrl, termsOfService],
        },
      ];
    }
    ...
    content = [
      {
        text: bookTitle || localize.apiReference,
        style: ["h2", "primary", "right", "b", "topMargin1"],
      },
      spec.info.title
        ? { text: spec.info.title, style: ["title", "right"] }
        : "",
      spec.info.version
        ? {
            text: `${localize.apiVersion}: ${spec.info.version}`,
    ...
    for (const key in spec.securitySchemes) {
      tableContent.push([
        ...
        spec.securitySchemes[key].type +
          (spec.securitySchemes[key].scheme
            ? `, ${spec.securitySchemes[key].scheme}`
            : "") +
          (spec.securitySchemes[key].bearerFormat
            ? `, ${spec.securitySchemes[key].bearerFormat}`
            : ""),
        spec.securitySchemes[key].description
          ? spec.securitySchemes[key].description
          : "",
      ]);
    }
...
function getParameterTableDef(
  ...
  const tableContent = [
    [
      ...
      {
        text: includeExample ? localize.example : "",
        ...
        },
        {
          stack: [
            ...
            paramSchema.constrain
              ? { text: paramSchema.constrain, style: ["small", "gray"] }
              : "",
            ...
            paramSchema.pattern
              ? {
                  text: `${localize.pattern}: ${paramSchema.pattern}`,
                  style: ["small", "gray"],
                }
              : "",
          ],
  ...
  return [
    {
      text: `${paramType} ${localize.parameters}`.toUpperCase(),
      style: ["small", "b"],
      margin: [0, 10, 0, 0],
    },
    {
...
function getExamplesDef(contentTypeObj, localizedExampleLabel) {
  ...
  if (contentTypeObj.example) {
    exampleSectionDef.push([
      ...
      {
        text: JSON.stringify(contentTypeObj.example, null, "\u200B \u200B"),
        margin: [40, 10, 0, 0],
        style: "monoSub",
      },
    ]);
  ...
  if (contentTypeObj.examples) {
    ...
    for (const oneExample in contentTypeObj.examples) {
      exampleSectionDef.push([
        ...
        {
          text: JSON.stringify(oneExample, null, 2),
          margin: [40, 10, 0, 0],
          style: "monoSub",
        },
      ]);
...
function getRequestBodyDef(
  ...
  for (const contentType in requestBody.content) {
    ...
    const requestBodyDef = [
      {
        text: `${localize.requestBody} - ${contentType}`,
        margin: [0, 10, 0, 0],
        style: ["small", "b"],
      },
    ];
    ...
    if (
      (contentType.includes("form") ||
        contentType.includes("multipart-form")) &&
      contentTypeObj.schema
    ) {
      formParamDef = getParameterTableDef(
      ...
      if (origSchema) {
        ...
        if (schemaStyle === "object") {
          let treeDef;
          if (
            schemaInObjectNotaion["::type"] &&
            schemaInObjectNotaion["::type"] === "array"
          ) {
            treeDef = objectToTree(
              schemaInObjectNotaion["::props"],
              localize,
              "array"
            );
          } else {
          ...
          if (
            ...
            schemaTableTreeDef = objectToTableTree(
              schemaInObjectNotaion["::prop"],
              localize,
              "array"
            );
          } else {
            schemaTableTreeDef = objectToTableTree(
              schemaInObjectNotaion,
              localize
            );
          }
          if (
            schemaTableTreeDef &&
            schemaTableTreeDef.length > 0 &&
            Array.isArray(schemaTableTreeDef[0]) &&
            schemaTableTreeDef[0].length > 0
          ) {
...
function getResponseDef(
  ...
  for (const statusCode in responses) {
    ...
    for (const contentType in responses[statusCode].content) {
      const responseDef = [
        {
          text: `${localize.responseModel} - ${contentType}`,
          margin: [10, 10, 0, 0],
          style: ["small", "b"],
        },
      ];
      ...
      if (origSchema) {
        ...
        if (schemaStyle === "object") {
          ...
          if (
            ...
            rootObjectType = [
              {
                text: "ARRAY OF OBJECT WITH BELOW STRUCTURE",
                style: ["sub", "b", "alternate"],
                colSpan: 3,
              },
            ];
            ...
            rootObjectType = [
              {
                text: "OBJECT WITH BELOW STRUCTURE",
                style: ["sub", "b", "alternate"],
                colSpan: 3,
              },
            ];
    ...
    respDef.push({
      text: [
        {
          text: `${localize.statusCode} - ${statusCode}: `,
          style: ["small", "b"],
        },
  ...
  spec.tags.map((tag) => {
    ...
    for (let j = 0; j < tag.paths.length; j++) {
      ...
      operationContent.push({
        text: `${tagSeq + 1}.${pathSeq} ${path.method.toUpperCase()} ${
          path.path
        }`,
      ...
      const pathParams = path.parameters
        ...
        : null;
      const queryParams = path.parameters
        ? path.parameters.filter((param) => param.in === "query")
        : null;
      const headerParams = path.parameters
        ? path.parameters.filter((param) => param.in === "header")
        : null;
      const cookieParams = path.parameters
        ? path.parameters.filter((param) => param.in === "cookie")
        : null;

      const pathParamTableDef = getParameterTableDef(
        pathParams,
        "path",
        localize,
        includeExample
      );
      const queryParamTableDef = getParameterTableDef(
        queryParams,
        "query",
        localize,
        includeExample
      );
      const requestBodyTableDefs = getRequestBodyDef(
        path.requestBody,
        schemaStyle,
        localize,
        includeExample
      );
      const headerParamTableDef = getParameterTableDef(
        headerParams,
        "header",
        localize,
        includeExample
      );
      const cookieParamTableDef = getParameterTableDef(
        cookieParams,
        "cookie",
        localize,
        includeExample
      ...
      if (
        ...
        requestSetDef.push({
          text: localize.noRequestParameters,
          style: ["small", "gray"],
          margin: [0, 5, 0, 0],
        });
      }
...
export function getApiListDef(spec, sectionHeading, localize) {
  const content = [
    { text: sectionHeading, style: ["h3", "b"], pageBreak: "none" },
  ];
  spec.tags.map((tag, i) => {
    const tableContent = [
      [
